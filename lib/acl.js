...
/**
  addRole( role, attributes, function(err) )
  ...
  @param {Function} Callback called when finished.
*/
Acl.prototype.setRoleAttribute = function(role, attribute, value, cb){
  // contract(arguments).params('string', 'object', 'function').end();
  
  var transaction = this.backend.begin();
  this.backend.add(transaction, 'roles', role, 'attribute');
  this.backend.end(transaction, cb);
};

/**
  setRoleAttribute( role, attribute, value, function(err) )
  
  Retrieves an attribute for a role from the system.
  
  @param {String} Role to change attribute.
  @param {String} Attribute to be changed.
  @param {String} New value for the specified attribute.
  @param {Function} Callback called when finished.
*/
Acl.prototype.setRoleAttribute = function(role, attribute, value, cb){
  contract(arguments).params('string', 'string', 'string', 'function').end();
  
  var transaction = this.backend.begin();
  this.backend.add(transaction, 'roles', role + '@' + attribute, value);
  this.backend.end(transaction, cb);
};

/**
  getRoleAttribute( role, attribute, function(err) )
  
  Removes a resource from the system
  
  @param {String} Role to retrieve attribute.
  @param {String} Attribute to be retrieved.
  @param {Function} Callback called when finished.
*/
Acl.prototype.getRoleAttribute = function(role, attribute, cb){
  contract(arguments).params('string', 'string', 'function').end();
  
  this.backend.get('roles', role + '@' + attribute, function(err, value){
    if(err) {
      cb(err);
    }else{
      cb(undefined, value);
